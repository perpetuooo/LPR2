...
public class FormPessoa extends JFrame implements ActionListener {
    ...
    public FormPessoa() {
        ...
        rM = new JRadioButton("M");
        rF = new JRadioButton("F");
        g = new ButtonGroup();
        g.add(rM);
        g.add(rF);
        JPanel pSexo = new JPanel(new FlowLayout(FlowLayout.LEFT, 5, 0));
        pSexo.add(rM);
        pSexo.add(rF);
        ...
        c.gridx = 1; c.gridy = 2; form.add(pSexo, c);
    ...
    private Character sexoEscolhido() {
        if (rM.isSelected()) return 'M';
        if (rF.isSelected()) return 'F';
        return null;
    }
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == bOk) {
            try {
                ...
                Character sx = sexoEscolhido();
                String idt = fIdade.getText().trim();
                if (nome.isEmpty() || sx == null || idt.isEmpty()) throw new Exception();
                int idade = Integer.parseInt(idt);
                UmaPessoa.setNome(nome);
                UmaPessoa.setSexo(sx);
